...
export class KafkaClient implements OnApplicationBootstrap, OnModuleDestroy {
  //   private readonly _logger = new Logger(KafkaClient.name);
  private kafka: Kafka;
  ...
  > = new Map();
  private readonly messageHandlers: Map<string, CallableFunction> = new Map();
        ...
        eachMessage: async (payload: EachMessagePayload) => {
          ...
          if (match) {
            try {
              ...
              this._logger.error(`Error handling message on ${topic}`, { err });
            }
          }
        },
      });
  ...
  async publish<T = any>(data: T, options: PublishOptions): Promise<void> {
    try {
      ...
      this._logger.debug(`Message published to topic ${options.topic}`);
    } catch (error) {
      this._logger.error(
  ...
  public async ping(): Promise<void> {
    try {
      ...
      this._logger.error("Kafka ping failed", { error });
      throw error;
    }
  }
}
