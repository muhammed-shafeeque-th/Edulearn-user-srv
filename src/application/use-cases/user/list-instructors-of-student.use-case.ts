  ...
  async execute(
    ...
    return this.tracer.startActiveSpan(
      ...
      async (span) => {
        ...
        const { data, total } =
          await this.instructorStudentRepository.getInstructorsOfStudent({ studentId, pagination: { page, limit } });
        ...
        const users = await  this.userRepository.findUsersByIds(instructorIds);
          ...
          this.logger.info(
            "ListInstructorsOfStudentUseCase succeeded.",
            {
              studentId,
          ...
          this.logger.error("Error in ListInstructorsOfStudentUseCase: " + (error?.message || error), error);
          throw error;
        }
      }
    );
