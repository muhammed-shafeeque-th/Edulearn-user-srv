import UnBlockInstructorRoleUseCaseImpl from "@/application/use-cases/profile/impls/unblock-instructor.use-case";
...
describe("UnBlockInstructorRoleUseCaseImpl", () => {
  let useCase: UnBlockInstructorRoleUseCaseImpl;
  ...
  beforeEach(() => {
    ...
    useCase = new UnBlockInstructorRoleUseCaseImpl(
      userRepo,
  ...
  it("should unblock instructor role and publish Kafka event", async () => {
  ...
  it("should return user without change if not blocked", async () => {
    const mockUser = createMockUser({
      id: "instructor-1",
      roles: [UserRoles.INSTRUCTOR],
    });
  ...
  it("should log unblocking operation", async () => {
    const mockUser = createMockUser({
      id: "instructor-1",
      roles: [UserRoles.INSTRUCTOR],
    });
  ...
  it("should publish InstructorRoleUnBlockedEvent", async () => {
    const mockUser = createMockUser({
      id: "instructor-1",
      roles: [UserRoles.INSTRUCTOR],
    });
